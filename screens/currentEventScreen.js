...
import { Title, List, RadioButton } from "react-native-paper";
    ...
    return (
      <View style={{ alignItems: "center" }}>
        <TouchableOpacity
          activeOpacity={0.5}
          onPress={() => {
            window.events[window.eventID].deleteEvent();
            navigation.navigate(screenName);
          }}
        >
          <Icon name="trash-can-outline" size={50} />
        </TouchableOpacity>
      </View>
    );
  }
  ...
  for (let i = 0; i < window.events[window.eventID].tournaments.length; i++) {
    tournaments.push(
      <View>
        ...
        >
          <TournamentButton
            name={event.tournaments[i].name}
            img={event.tournaments[i].game}
            date={event.tournaments[i].date}
            endDate={event.tournaments[i].endDate}
            EID={event.tournaments[i].EID}
            TID={event.tournaments[i].TID}
            deleted={event.tournaments[i].deleted}
          ></TournamentButton>
        </View>
        <View style={styles.separator} />
      </View>
    ...
    <ScrollView>
              ...
              <View style={{ marginLeft: 5 }}>
                <List.Section title="Pick Game">
                  ...
                  >
                    ...
                    />
                    <List.Item
                      title="Blazeblue Centralfiction"
                      onPress={changeGame6}
                    />
                  </List.Accordion>
                </List.Section>
                ...
                <View style={{ paddingTop: 15 }}>
                  <Button
                    ...
                    color="#f194ff"
                    onPress={() => {
                      event.addTournament(tName, tDesc, date, endDate, game);
      ...
      <Dialog.Container visible={visible4}>
        ...
        <Dialog.Input
          ...
          value={cohostName}
          onChangeText={onChangeCohostName}
        ></Dialog.Input>
        <Dialog.Button label="Cancel" onPress={handleCancel} />
